    ...
    const updateFormField = useCallback((field: keyof LoanFormData, value: string) => {
        setFormData((prev) => {
            ...
            if (field === 'loanProduct') {
                ...
                if (product) {
                    if (newData.loanAmount === '0' || newData.loanAmount === '') {
                        newData.loanAmount = product.loan_amount.toString();
                    }
                    if (newData.requestedAmount === '0' || newData.requestedAmount === '') {
                    ...
                    // @ts-ignore - calculated_rental is number? in types but we use string in logic sometimes
