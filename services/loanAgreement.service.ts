...
export const loanAgreementService = {
    ...
    /**
     * Download the loan agreement document (Word file)
     */
    downloadAgreement: async (id: number | string): Promise<void> => {
        const response = await fetch(`${API_BASE_URL}/loan-agreements/${id}/download`, {
            headers: getHeaders()
        });

        if (!response.ok) {
            const json = await response.json().catch(() => ({}));
            throw new Error(json.message || 'Failed to download agreement. It may be locked.');
        }

        const blob = await response.blob();
        const contentDisposition = response.headers.get('Content-Disposition');
        let fileName = `Loan_Agreement_${id}.docx`;
