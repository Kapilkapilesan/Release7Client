...
type TabType = 'promotion' | 'increment';
        ...
        try {
            ...
            if (requestCategory === 'promotion') {
                ...
                if (actionType === 'approve') {
                    ...
                    toast.success('Salary increment approved!');
                } else {
            ...
            <div className="relative z-10 max-w-[1500px] mx-auto space-y-8 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                        ...
                        { label: 'Pending Increments', value: incrementRequests.length, icon: ArrowUpRight, color: colors.emerald[500] }
                    ].map((stat, idx) => (
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 p-1 bg-gray-100/50 rounded-2xl border border-gray-200/50 w-fit">
                        {[
                            ...
                            { id: 'increment', label: 'Increment Path', icon: DollarSign }
                        ].map((tab) => (
                            <button
                                key={tab.id}
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    <thead className="bg-gray-50/50">
                                        <tr className="border-b border-gray-100">
                                            ...
                                            {activeTab === 'promotion' ? (
                                                ...
                                                <>
                                                    <th className="px-4 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Active Base</th>
                                                    <th className="px-4 py-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Value Delta</th>
                                                </>
                                            )}
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {(activeTab === 'promotion' ? filteredPromotionRequests : filteredIncrementRequests).map((request) => (
                                            <tr key={request.id} className="group hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-6">
                                                    {activeTab === 'promotion' ? (
                                                        ...
                                                        <p className="text-[11px] font-black text-gray-400 tabular-nums">{formatCurrency((request as SalaryIncrementRequest).current_salary)}</p>
                                                    )}
                                                </td>
                                                <td className="px-4 py-6">
                                                    <div className="flex items-center gap-2">
                                                        <ArrowUpRight className="w-3.5 h-3.5 text-emerald-500" />
                                                        <span className={`text-[11px] font-black uppercase ${activeTab === 'promotion' ? 'text-primary-600' : 'text-emerald-600'}`}>
                                                            {activeTab === 'promotion' ? (request as PromotionRequest).requested_role_name : `+${formatCurrency((request as SalaryIncrementRequest).requested_amount)}`}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-6">
                                                ...
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            onClick={() => openActionModal(request, 'approve', activeTab)}
                                                        ...
                                                        <button
                                                            onClick={() => openActionModal(request, 'reject', activeTab)}
                                        ...
                                        {(activeTab === 'promotion' ? filteredPromotionRequests : filteredIncrementRequests).length === 0 && (
                                            <tr>
                            ...
                            <div className="bg-white rounded-xl p-5 border border-gray-100 space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <p className={`text-[10px] font-black uppercase ${actionType === 'approve' ? 'text-emerald-600' : 'text-rose-600'}`}>
                                        {requestCategory === 'promotion' ? (selectedRequest as PromotionRequest).requested_role_name : `+${formatCurrency((selectedRequest as SalaryIncrementRequest).requested_amount)}`}
                                    </p>
                                </div>
                            </div>
                        </div>
