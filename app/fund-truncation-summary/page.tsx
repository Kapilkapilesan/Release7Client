    ...
    return (
        <div className="min-h-screen relative overflow-hidden" style={{ backgroundColor: colors.surface.background }}>
            ...
            <div className="relative z-10 p-6 max-w-[1600px] mx-auto space-y-6 animate-in fade-in duration-700">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/70 backdrop-blur-xl p-6 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-white/50">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-gray-900 tracking-tight uppercase leading-none">Truncation Summary</h1>
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="flex bg-gray-50/50 p-1.5 rounded-2xl border border-gray-100/50 backdrop-blur-sm">
                        ...
                        {period !== 'all' && (
                            <div className="flex items-center gap-2 bg-gray-50/50 p-1.5 rounded-2xl border border-gray-100/50">
                                <div className="relative group">
                                    <Calendar className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none group-focus-within:text-primary-500 transition-colors" />
                                    <input
                                        ...
                                        className="pl-9 pr-4 py-2.5 bg-white border-none rounded-xl text-[10px] font-black uppercase tracking-widest outline-none ring-1 ring-gray-100 hover:ring-primary-300 transition-all w-44 text-gray-900"
                                    />
                                </div>
                            </div>
                        )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={i} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
                            <div className="relative z-10 flex flex-col gap-2">
                                <span className="text-gray-400 text-[10px] font-black uppercase tracking-widest opacity-60 group-hover:opacity-100 transition-opacity">{stat.label}</span>
                                <span className="text-3xl font-black text-gray-900 tabular-nums leading-none">{stat.value}</span>
                            </div>
                            <stat.icon
                ...
                {/* Main Content Area */}
                <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/50 overflow-hidden flex flex-col min-h-[600px]">
                    {/* Balanced Tabs */}
                    <div className="flex flex-wrap items-center bg-gray-50/30 px-6 pt-6 gap-8 border-b border-gray-50">
                        {[
                            ...
                            >
                                <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? 'text-primary-600' : 'text-gray-300 group-hover:text-gray-400'}`} />
                                {tab.label}
                                {activeTab === tab.id && (
                        ...
                        <div className="ml-auto mb-5 relative group">
                            <SearchCode className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none group-focus-within:text-primary-600 transition-colors" />
                            <input
                                type="text"
                                placeholder="Quick search catalog..."
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="pl-11 pr-5 py-3 bg-gray-100/50 border-none rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none ring-1 ring-gray-200 focus:ring-4 focus:ring-primary-500/10 w-72 transition-all"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-0 overflow-x-auto custom-scrollbar">
                        {isLoading ? (
                            <div className="h-full flex flex-col items-center justify-center py-20 gap-4">
                                ...
                                <p className="text-[11px] text-gray-400 font-black uppercase tracking-widest animate-pulse">Filtering your summary...</p>
                            </div>
                        ) : filteredRecords.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center py-24 gap-6">
                                <div className="p-8 bg-gray-50 rounded-full shadow-inner ring-8 ring-gray-50/50">
                                    <Search className="w-12 h-12 text-gray-200" />
                                </div>
                                <div className="text-center">
                                    <h3 className="text-gray-900 font-black uppercase text-xs tracking-widest">No matching records found</h3>
                                    <p className="text-gray-400 text-[10px] font-bold uppercase tracking-wider mt-2 opacity-60">Try adjusting your filters or refining your search</p>
                                </div>
                            </div>
                        ) : (
                            <table className="w-full text-left border-separate border-spacing-0">
                                <thead>
                                    <tr className="bg-gray-50/20">
                                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-[0.2em] border-b border-gray-50 whitespace-nowrap">Date & Time</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-[0.2em] border-b border-gray-50 whitespace-nowrap">Recipient Entity</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-[0.2em] text-right border-b border-gray-50 whitespace-nowrap">Allocated Amount</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-[0.2em] border-b border-gray-50 whitespace-nowrap">Registry / Method</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase text-gray-400 tracking-[0.2em] border-b border-gray-50 whitespace-nowrap">Department</th>
                                        <th className="px-8 py-5 border-b border-gray-50"></th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50/50">
                                    {filteredRecords.map((rec: any, idx: number) => (
                                        <tr key={idx} className="group hover:bg-gray-50/30 transition-all duration-500">
                                            <td className="px-8 py-5">
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <span className="text-[9px] text-gray-400 font-black uppercase tracking-widest opacity-60">Verified Output</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5">
                                                <div className="flex flex-col">
                                                    <span className="text-base font-black text-gray-900 uppercase tracking-tight leading-none group-hover:text-primary-600 transition-colors">
                                                        {activeTab === 'loans' ? rec.customer?.full_name : activeTab === 'investments' ? rec.investment?.customer?.full_name : rec.staff?.full_name}
                                                    </span>
                                                    <span className="text-[10px] text-gray-400 font-bold mt-2 tracking-widest uppercase">
                                            ...
                                            <td className="px-8 py-5 text-right">
                                                <div className="flex flex-col gap-1">
                                                    <span className="text-lg font-black text-gray-900 tabular-nums tracking-tighter">
                                                    ...
                                                    <div className="flex items-center justify-end gap-1.5 px-2 py-0.5 bg-emerald-50 rounded-lg w-fit ml-auto border border-emerald-100/50">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                                        <span className="text-[9px] text-emerald-600 font-black uppercase tracking-widest">Finalized</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5">
                                                <div className="flex flex-col gap-1">
                                                    <span className="text-xs font-black text-gray-700 tracking-widest uppercase leading-none">
                                                    ...
                                                    <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest opacity-60">System Protocol</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5">
                                                <span className="px-3 py-1.5 bg-gray-50 text-gray-500 rounded-lg text-[9px] font-black uppercase tracking-widest border border-gray-100 group-hover:bg-primary-50 group-hover:text-primary-600 group-hover:border-primary-100 transition-all duration-300">
                                                    {(activeTab === 'loans' ? rec.center?.branch?.branch_name : rec.staff?.branch?.branch_name) || 'Head Office'}
                                                </span>
                                            </td>
                                            <td className="px-8 py-5 text-right">
                                                <button className="p-2 text-gray-300 hover:text-primary-600 transition-all opacity-0 group-hover:opacity-100 transform translate-x-2 group-hover:translate-x-0">
                    ...
                    {/* Proportional Footer */}
                    <div className="px-8 py-6 bg-gray-50/20 border-t border-gray-50 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl shadow-sm border border-gray-100">
                                ...
                                <span className="text-[9px] text-gray-400 font-black uppercase tracking-widest">Audit Protocols Active</span>
                            </div>
                        </div>
                    </div>
                </div>
