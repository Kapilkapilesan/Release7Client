...
export function SalaryDisbursementTable({ records, onDisburse, onBulkDisburse }: Props) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-gray-50/40 p-2 rounded-2xl border border-gray-100/50 backdrop-blur-md space-y-2 shadow-[0_5px_15px_-5px_rgba(0,0,0,0.02)]">
                <div className="flex flex-wrap gap-3 items-center">
                    <div className="flex-1 min-w-[300px] flex items-center gap-3 bg-white px-4 py-2.5 rounded-xl shadow-sm border border-gray-100/50 focus-within:ring-4 focus-within:ring-primary-500/10 transition-all duration-300 group">
                    ...
                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100 group">
                        <Filter className="w-3.5 h-3.5 text-primary-500" />
                        <select
                            value={roleFilter}
                            onChange={(e) => setRoleFilter(e.target.value)}
                            className="bg-transparent outline-none text-[9px] font-black text-gray-600 uppercase tracking-widest min-w-[120px] cursor-pointer appearance-none text-center"
                        >
                    ...
                    <table className="w-full text-left border-separate border-spacing-0">
                        <thead>
                            <tr className="bg-gray-50/40">
                                <th className="px-4 py-4 w-12 border-b border-gray-100/50 text-center">
                                    <button
                                        onClick={toggleSelectAll}
                                        className="text-gray-400 hover:text-primary-600 transition-all bg-white w-8 h-8 rounded-lg border border-gray-100 flex items-center justify-center shadow-sm hover:shadow-md active:scale-95"
                                    >
                                        {isAllSelected ? <CheckSquare className="w-4 h-4 text-primary-600" /> : <Square className="w-4 h-4" />}
                                    </button>
                                </th>
                                <th className="px-4 py-4 text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100/50 whitespace-nowrap">Asset Identity</th>
                                <th className="px-4 py-4 text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100/50 whitespace-nowrap">Operating Role</th>
                                <th className="px-4 py-4 text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100/50 text-center whitespace-nowrap">Cycle</th>
                                <th className="px-4 py-4 text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100/50 text-center whitespace-nowrap">Net Allocation</th>
                                <th className="px-4 py-4 text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100/50 text-center whitespace-nowrap">Registry Status</th>
                                <th className="px-4 py-4 text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] border-b border-gray-100/50 text-right whitespace-nowrap">Control</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50/80">
                            {filteredRecords.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="px-4 py-20 text-center bg-gray-50/20">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.2em]">No personnel records detected</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                filteredRecords.map((record) => (
                                    <tr key={record.id} className={`group hover:bg-white/40 transition-all duration-300 ${selectedIds.has(record.id.toString()) ? 'bg-primary-50/10' : ''}`}>
                                        <td className="px-4 py-4 text-center">
                                            {record.status !== 'Paid' && record.status !== 'Disbursed' && (
                                                <button
                                                    onClick={() => toggleSelect(record.id.toString())}
                                                    className="text-gray-400 hover:text-primary-600 transition-all hover:scale-110 active:scale-90"
                                                >
                                        ...
                                        <td className="px-4 py-4">
                                            <div>
                                                <p className="text-sm font-black text-gray-900 group-hover:text-primary-600 transition-colors uppercase tracking-tight leading-none mb-1">{record.staff?.full_name || 'System Resource'}</p>
                                                <div className="flex items-center gap-1.5">
                                                    <div className="w-1 h-1 rounded-full bg-gray-200 group-hover:bg-primary-400 transition-colors" />
                                                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">{record.staff?.branch?.branch_name || record.staff?.branch?.name || 'CENTRAL'}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4">
                                            <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest bg-gray-50/50 px-3 py-1 rounded-lg border border-gray-100/50 group-hover:bg-white transition-colors">
                                                {record.staff?.work_info?.designation || record.staff?.role || 'RESOURCE'}
                                            </span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                <span className="text-[9px] font-black text-indigo-600 uppercase tracking-[0.2em]">{record.month}</span>
                                                <div className="w-6 h-[1px] bg-indigo-100 rounded-full" />
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex flex-col items-center">
                                                <span className="text-lg font-black text-gray-900 tracking-tighter tabular-nums leading-none mb-1 group-hover:scale-105 transition-transform duration-300">
                                                    {Number(record.net_payable).toLocaleString()}
                                                </span>
                                                <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Net</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex flex-col items-center">
                                        ...
                                        <td className="px-4 py-4 text-right">
                                            {record.status !== 'Disbursed' && record.status !== 'Paid' ? (
                                                ...
                                                <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] border border-emerald-100/50 shadow-inner ml-auto">
